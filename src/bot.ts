...
async function renderInstructionsPanel(ctx: Context) {
  await upsertPanel(
    ...
    uiCard("📖 Как подключиться", [
      ...
      "Если к странице подписки перейти не удаётся, по инструкции на ней подключиться не получается, ссылка не открывается или непонятно, что делать — установите Happ и вставьте в него ту же ссылку-ключ.",
