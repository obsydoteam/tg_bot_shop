...
async function renderInstructionsPanel(ctx: Context) {
  await upsertPanel(
    ...
    uiCard("📖 Как подключиться", [
      "1) «Мой профиль» → скопируйте ссылку-ключ подписки.",
      "2) Откройте ссылку в браузере и следуйте шагам на странице подписки: импорт подписки в клиент и подключение.",
      "",
      "Запасной вариант — Happ (App Store / Google Play):",
      "Если по странице не получается подключиться, ссылка не открывается или непонятно, что делать — установите Happ и вставьте в него ту же ссылку-ключ подписки.",
      "В Happ: справа вверху «+» → «URL подписки». В открывшемся меню прокрутите вниз, вставьте ссылку-ключ и нажмите «Готово»."
    ]),
    Markup.inlineKeyboard([
...
bot.command("help", async (ctx) => {
  await ctx.reply(
    uiCard("📚 Команды", [
      ...
      "В меню — «Инструкция»: ключ в профиле, страница подписки в браузере; Happ — запасной вариант с импортом по URL.",
      "",
      "Для админа: /admin"
    ])
  );
