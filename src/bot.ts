...
async function renderInstructionsPanel(ctx: Context) {
  await upsertPanel(
    ...
    uiCard("📖 Как подключиться", [
      "Сначала настраиваем подключение через страницу подписки по ссылке-ключу. Happ — только если так не получилось.",
      "",
      "1) «Мой профиль» → скопируйте ссылку-ключ подписки (платный или trial).",
      "2) Откройте ссылку в браузере и следуйте шагам на странице подписки: импорт подписки в клиент и включение.",
      "3) Запасной вариант: если по странице не получается подключиться, ссылка не открывается или непонятно, что делать — установите Happ (App Store / Google Play) и вставьте в него ту же ссылку подписки."
    ]),
    Markup.inlineKeyboard([
...
bot.command("help", async (ctx) => {
  await ctx.reply(
    uiCard("📚 Команды", [
      ...
      "В меню — «Инструкция»: сначала страница подписки по ссылке-ключу; Happ — если без приложения не вышло.",
      "",
      "Для админа: /admin"
    ])
  );
