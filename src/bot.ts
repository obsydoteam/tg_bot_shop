...
async function renderInstructionsPanel(ctx: Context) {
  await upsertPanel(
    ...
    uiCard("📖 Как подключиться", [
      "1) Откройте «Мой профиль» и скопируйте ссылку-ключ подписки (платный или trial).",
      "2) Откройте ссылку в браузере: на странице подписки будет краткая инструкция по импорту подписки в приложение.",
      "3) Установите Happ (App Store / Google Play) и добавьте эту ссылку подписки в приложение — так проще всего подключиться."
    ]),
    Markup.inlineKeyboard([
...
bot.command("help", async (ctx) => {
  await ctx.reply(
    uiCard("📚 Команды", [
      ...
      "В меню — «Инструкция»: ключ в «Мой профиль», на странице подписки — импорт подписки, дальше удобнее через Happ.",
      "",
      "Для админа: /admin"
    ])
  );
