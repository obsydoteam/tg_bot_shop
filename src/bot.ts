...
async function renderInstructionsPanel(ctx: Context) {
  await upsertPanel(
    ...
    uiCard("📖 Как подключиться", [
      ...
      "2) Откройте ссылку в браузере: на странице подписки будет краткая инструкция по импорту профиля.",
      "3) Рекомендуем установить приложение Happ (App Store / Google Play) и добавить туда эту ссылку подписки — так проще всего подключиться."
    ]),
    Markup.inlineKeyboard([
